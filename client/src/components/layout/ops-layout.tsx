...
const NAV_ITEMS = [
  { path: "/", label: "Command Center", icon: "grid" },
...
const ICONS: Record<string, ReactNode> = {
  ...
  target: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 12m-9 0a9 9 0 1018 0 9 9 0 10-18 0m9-5a5 5 0 110 10 5 5 0 010-10m0 3a2 2 0 110 4 2 2 0 010-4" /></svg>,
  ...
  "file-text": <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>,
  ...
  stethoscope: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19 14.5M14.25 3.104c.251.023.501.05.75.082M19 14.5a2.25 2.25 0 012.25 2.25v.75a2.25 2.25 0 01-4.5 0v-.75A2.25 2.25 0 0119 14.5z" /></svg>,
  link: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-3.122a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L5.25 9.75" /></svg>,
  ...
  shield: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12.75L11.25 15 15 9.75M21 12c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9 9 4.03 9 9z" /></svg>,
  funnel: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 4.5h18l-7 9v6l-4 2v-8L3 4.5z" /></svg>,
};
  ...
  return (
    <div className="flex h-screen">
      ...
      <aside className="w-64 bg-ops-surface border-r border-ops-border flex flex-col shrink-0">
        {/* Logo */}
        <div className="h-16 flex items-center px-6 border-b border-ops-border gap-2">
          <img
            src={logoWhite}
            alt="FITSCRIPT"
          ...
          />
          <span className="text-xs text-ops-text-muted font-medium bg-ops-bg px-2 py-0.5 rounded">
            OPS
          </span>
        </div>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.path} href={item.path}>
                <div
                  className={`flex items-center gap-3 px-6 py-2.5 text-sm cursor-pointer transition-colors ${
                    isActive
                      ? "text-fitscript-green bg-fitscript-green/8 border-r-2 border-fitscript-green"
      ...
      <main className="flex-1 overflow-y-auto">
        {/* Top Bar */}
        <header className="h-16 border-b border-ops-border flex items-center justify-between px-8 bg-ops-surface/50 backdrop-blur sticky top-0 z-10">
          <div />
          <div className="flex items-center gap-4">
            <div className="w-2 h-2 rounded-full bg-fitscript-green animate-pulse" />
            <span className="text-sm text-ops-text-muted">Live</span>
            ...
            {adminEmail && (
              <>
                <div className="w-px h-5 bg-ops-border" />
                <span className="text-xs text-ops-text-muted truncate max-w-[180px]" title={adminEmail}>
                  {adminEmail}
                </span>
                <button
                  onClick={logout}
