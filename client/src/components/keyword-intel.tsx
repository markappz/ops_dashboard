      ...
      {data?.clusters?.length ? (
        <div className="overflow-x-auto rounded-xl border border-ops-border bg-ops-surface shadow-card">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-ops-border text-left text-[11px] uppercase tracking-wider text-ops-text-muted">{["Priority", "Pillar", "Intent", "Page type", "Keywords", "Covered", "Impr 90d"].map((h) => <th key={h} className="px-4 py-3 font-medium">{h}</th>)}</tr></thead>
            <tbody>
              {data.clusters.map((c) => {
                ...
                return (
                  <>
                    ...
                    {open === c.id && (
                      <tr key={`${c.id}-kw`}><td colSpan={7} className="bg-ops-bg/40 p-0">
