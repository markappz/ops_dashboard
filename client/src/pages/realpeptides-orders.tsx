...
const CHANNELS: Record<string, { label: string; icon: React.ReactNode; cls: string; what: string }> = {
  ...
  direct: { label: "Direct", icon: <CircleDot size={13} />, cls: "bg-ops-border text-ops-text-muted", what: "No referrer and no UTMs: typed the URL, used a bookmark, or came from an app (Instagram, X, TikTok, iMessage, email apps) that hid where they came from. Untagged social lands here, and so does some organic search and AI-assistant traffic when the browser or assistant drops the referrer — treat Direct as a floor for those channels, not as a channel of its own." },
