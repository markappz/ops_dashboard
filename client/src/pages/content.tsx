...
/**
 * Which brand's Clomark business profile this page talks to. The FitScript
 * /content page leaves it at the default; pawgen / Real Peptides / PeptideU
 * mount the same section with their own company so every query, mutation and
 * cache key is scoped per brand.
 */
export type ClomarkCompany = "fitscript" | "pawgen" | "realpeptides" | "peptideu";
const ClomarkCompanyContext = createContext<ClomarkCompany>("fitscript");

function useClomark() {
  const company = useContext(ClomarkCompanyContext);
  const url = (path: string) => `${path}${path.includes("?") ? "&" : "?"}company=${company}`;
  return { company, url };
}

/** Clomark content workflow for a non-FitScript brand, as its own page. */
export function CompanyContent({ company, label }: { company: ClomarkCompany; label: string }) {
  return (
    <ClomarkCompanyContext.Provider value={company}>
      <div>
        <PageHero
          eyebrow={label}
          title="Content"
          subtitle={company === "realpeptides"
            ? "What is live on realpeptides.co, then the Clomark generation queue for this brand."
            : "SEO pages and blog posts generated by Clomark for this brand — queue, drafts, approvals and publishing."}
        />
...
function ClomarkSection() {
  ...
  const { data: status } = useQuery<ClomarkStatus>({
    queryKey: ["ops-clomark-status", cl.company],
    queryFn: () => fetch(cl.url("/api/ops/clomark/status")).then((r) => r.json()),
  });
  const ready = !!status?.connected && !!status.businessIdConfigured;

  const { data, isLoading, error } = useQuery<ClomarkOverview>({
    queryKey: ["ops-clomark-overview", cl.company],
    queryFn: () => fetch(cl.url("/api/ops/clomark/overview")).then((r) => r.json()),
    enabled: ready,
    staleTime: 60_000 * 5,
  });
  ...
  const { data: contentData } = useQuery<{
    ...
    queryKey: ["ops-clomark-content", cl.company],
    queryFn: () =>
      fetch(cl.url("/api/ops/clomark/content?limit=200")).then((r) => r.json()),
    enabled: ready,
    staleTime: 5_000,
    ...
    <div className="bg-ops-surface border border-ops-border rounded-xl p-5 shadow-card mb-6">
      {addOpen && (
        <AddContentModal
          ...
          onSaved={() => {
            queryClient.invalidateQueries({ queryKey: ["ops-clomark-overview", cl.company] });
            queryClient.invalidateQueries({ queryKey: ["ops-clomark-content", cl.company] });
            setAddOpen(false);
          }}
        />
      )}
...
function QueueTable({ suggestions }: { suggestions: ContentSuggestion[] }) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(cl.url(`/api/ops/clomark/content-suggestions/${id}`), {
        method: "DELETE",
      });
      if (!r.ok) {
...
function AddContentModal({
  ...
  const submitSingle = async () => {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(cl.url("/api/ops/clomark/content-suggestions"), {
        method: "POST",
  ...
  const submitBulk = async () => {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(cl.url("/api/ops/clomark/content-suggestions/bulk"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function ClomarkBody({ data }: { data: ClomarkOverview }) {
  ...
  // An audit older than 60 days is history, not a score - show it as such rather than as a live number.
  const scoreStale = !!score?.createdAt && Date.now() - new Date(score.createdAt).getTime() > 60 * 86_400_000;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-5">
        <KpiTile
          label="Clomark SEO audit"
          value={
            score?.overallScore !== undefined && !scoreStale
          ...
          sub={
            score?.createdAt
              ? scoreStale
                ? `stale — last audit ${new Date(score.createdAt).toLocaleDateString()}`
...
function LocationPageModal({
  ...
  const { data: opts } = useQuery<LocationOptions>({
    queryKey: ["ops-clomark-location-options", cl.company],
    queryFn: () =>
      fetch(cl.url("/api/ops/clomark/location/options")).then((r) => r.json()),
    staleTime: 60_000 * 60,
  });
  ...
  const { data: profileData } = useQuery<{ items: ProfileItem[] }>({
    queryKey: ["ops-clomark-profile-items", cl.company],
    queryFn: () => fetch(cl.url("/api/ops/clomark/profile-items")).then((r) => r.json()),
    staleTime: 60_000 * 10,
  });
  ...
  const lookupZips = async () => {
    ...
    try {
      ...
      const r = await fetch(cl.url(`/api/ops/clomark/location/zip-lookup?${qs}`));
      const body = await r.json();
  ...
  const submit = async () => {
    ...
    try {
      ...
      const r = await fetch(cl.url("/api/ops/clomark/location-page"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function ContentViewerModal({
  ...
  const { data, isLoading } = useQuery<{ content: FullContent }>({
    queryKey: ["ops-clomark-generated", cl.company, contentId],
    queryFn: () =>
      fetch(cl.url(`/api/ops/clomark/generated/${contentId}`)).then((r) => r.json()),
  });
...
function BulkPublishDialog({
  ...
  const { data: platformsData, isLoading: platformsLoading } = useQuery<{
    ...
    queryKey: ["ops-clomark-publishing-platforms", cl.company],
    queryFn: () =>
      fetch(cl.url("/api/ops/clomark/publishing/platforms")).then((r) => r.json()),
    staleTime: 60_000 * 5,
  });
