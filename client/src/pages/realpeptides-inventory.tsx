...
import {
  ...
  PackageOpen, AlertTriangle, CheckCircle2, Pencil,
} from "lucide-react";
import { PageHero } from "../components/page-hero";
import { API, api, ui, thumbUrl, type Sku } from "./coa/api";
import { downloadOrderPdf, isLow, orderQty, stockNum } from "./coa/order-pdf";

/**
 * Real Peptides Inventory — stock, labels, and the restock order.
 *
 * Same source of truth as the COA tab: the tracker's SKU registry. Stock
 * changes write through `/skus/:id/stock` (audit-logged) and surface on the
 * public feed (`stock`/`inStock`), which the new Vercel storefront reads.
 */

type Filter = "all" | "low" | "out" | "nolabel";
...
export default function RealPeptidesInventory() {
  ...
  const counts = useMemo(() => ({
    all: skus.length,
    low: skus.filter(isLow).length,
    out: skus.filter((s) => stockNum(s.current_stock) !== null && stockNum(s.current_stock)! <= 0).length,
    nolabel: skus.filter((s) => !s.label_doc_id).length,
    units: skus.reduce((a, s) => a + (stockNum(s.current_stock) ?? 0), 0),
  }), [skus]);

  const shown = useMemo(() => {
    let list = skus;
    if (filter === "low") list = list.filter(isLow);
    else if (filter === "out") list = list.filter((s) => stockNum(s.current_stock) !== null && stockNum(s.current_stock)! <= 0);
    else if (filter === "nolabel") list = list.filter((s) => !s.label_doc_id);
  ...
  function orderPdf() {
    const n = downloadOrderPdf(skus);
    say(n ? `Order PDF downloaded — ${n} product${n === 1 ? "" : "s"} below target.` : "Nothing is below target — no order needed.");
  }
  ...
  return (
    <div>
      <PageHero
        ...
        title="Inventory"
        subtitle={`${counts.all} products · ${counts.units.toLocaleString()} units on hand. Stock feeds the live site; every change is logged.`}
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={orderPdf} disabled={!skus.length} className={ui.primary}>
              <FileDown size={15} /> Order PDF{counts.low ? ` (${counts.low})` : ""}
            </button>
          </div>
        }
      />
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Kpi icon={<X size={17} />} label="Out of stock" value={counts.out} tone={counts.out ? "bad" : "ok"} active={filter === "out"} onClick={() => setFilter("out")} />
        <Kpi icon={<Tag size={17} />} label="Missing label" value={counts.nolabel} tone={counts.nolabel ? "warn" : "ok"} active={filter === "nolabel"} onClick={() => setFilter("nolabel")} />
      </div>
          ...
          <table className="w-full min-w-[880px] text-left text-sm">
            <thead>
              <tr className="border-b border-ops-border bg-ops-bg/40 text-[11px] uppercase tracking-wider text-ops-text-muted">
                ...
                <th className="px-4 py-3 text-right font-medium">To order</th>
            ...
            <tbody className="divide-y divide-ops-border/50">
              {shown.map((s) => (
                <Row key={s.id} sku={s} canEdit={canEdit} onChanged={refresh} onSay={say} />
              ))}
              {!shown.length && (
...
function Row({ sku, canEdit, onChanged, onSay }: { sku: Sku; canEdit: boolean; onChanged: () => void; onSay: (m: string) => void }) {
  const [qty, setQty] = useState("1");
  const [busy, setBusy] = useState(false);
  const [showLog, setShowLog] = useState(false);
  const labelRef = useRef<HTMLInputElement>(null);

  const cur = stockNum(sku.current_stock);
  const ideal = stockNum(sku.ideal_stock);
  const need = orderQty(sku);
  ...
  const adjust = (sign: 1 | -1) => {
    ...
    run(() => api(`/skus/${sku.id}/stock`, { method: "POST", body: JSON.stringify({ delta: n }) }),
      `${sku.product_name}: ${n > 0 ? "+" : ""}${n} → stock updates on the live site.`);
  };
  const setStock = () => {
    const v = prompt(`Set exact stock for ${sku.product_name} (${sku.sku_code}):`, String(cur ?? 0));
    if (v === null) return;
    const n = Number(v);
    if (!Number.isFinite(n) || n < 0) return onSay("Enter a non-negative number.");
    run(() => api(`/skus/${sku.id}/stock`, { method: "POST", body: JSON.stringify({ set: n, note: "manual count" }) }),
      `${sku.product_name}: counted at ${n}.`);
  };
  const setIdeal = () => {
    const v = prompt(`Restock target for ${sku.product_name} — the order PDF fills up to this:`, String(ideal ?? ""));
    if (v === null) return;
    run(() => api(`/skus/${sku.id}`, { method: "PATCH", body: JSON.stringify({ ideal_stock: v.trim() === "" ? null : Number(v) }) }),
      "Target saved.");
  };
  function onLabel(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    run(() => api("/documents", { method: "POST", body: fd }), `Label filed for ${sku.product_name}.`)
  ...
  return (
    <>
      <tr data-sku={sku.sku_code} className={out ? "bg-red-500/[0.04]" : low ? "bg-yellow-500/[0.04]" : undefined}>
        ...
        <td className="px-4 py-3">
          <input ref={labelRef} type="file" hidden onChange={onLabel} />
          {sku.label_doc_id ? (
            <span className="inline-flex items-center gap-2">
              ...
              {canEdit && <button type="button" onClick={() => labelRef.current?.click()} disabled={busy} title="Replace label" className="text-ops-text-muted hover:text-ops-text"><Upload size={13} /></button>}
            </span>
          ) : canEdit ? (
        ...
        <td className="px-4 py-3 text-right">
          <button type="button" onClick={canEdit ? setStock : undefined} title={canEdit ? "Click to set an exact count" : undefined}
        ...
        <td className="px-4 py-3 text-right">
          <button type="button" onClick={canEdit ? setIdeal : undefined}
        ...
        {canEdit && (
          <td className="px-4 py-3">
            <div className="flex items-center justify-center gap-1">
              <button type="button" onClick={() => adjust(-1)} disabled={busy || (cur ?? 0) <= 0} title="Remove stock"
              ...
              <button type="button" onClick={() => adjust(1)} disabled={busy} title="Add stock"
...
function LogRow({ skuId, colSpan }: { skuId: number; colSpan: number }) {
  const q = useQuery({
    ...
    queryFn: () => api<{ log: { id: number; delta: string; new_stock: string; note: string | null; changed_by: string | null; created_at: string }[] }>(`/skus/${skuId}/stock-log`),
  });
  return (
              ...
              {q.data.log.map((l) => {
                ...
                return (
                  <li key={l.id} className="flex flex-wrap items-center gap-2 text-ops-text-muted">
                    ...
                    <span className={`w-14 shrink-0 text-right font-semibold tabular-nums ${d > 0 ? "text-fitscript-green" : d < 0 ? "text-red-400" : "text-ops-text"}`}>
                      {d > 0 ? `+${d}` : d === 0 ? "set" : d}
                    </span>
